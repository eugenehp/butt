...
void button_connect_cb(void)
{
    ...
    if(cfg.main.silence_threshold > 0)
        Fl::add_timeout(cfg.main.silence_threshold, &stream_silence_timer);
    ...
    snprintf(text_buf, sizeof(text_buf), "Connected to: %s", cfg.srv[cfg.selected_srv]->name);
...
void button_record_cb(void)
{
    ...
    if (cfg.rec.silence_threshold > 0)
        Fl::add_timeout(cfg.rec.silence_threshold, &record_silence_timer);
...
void window_main_close_cb(void)
{
    ...
    if(connected || recording)
    {
        int ret;
        if(connected)
            ret = fl_choice("butt is currently streaming.\n"
                            "Do you really want to close butt now?",
                            "no", "yes", NULL);
        else
            ret = fl_choice("butt is currently recording.\n"
                            "Do you really want to close butt now?",
                            "no", "yes", NULL);
